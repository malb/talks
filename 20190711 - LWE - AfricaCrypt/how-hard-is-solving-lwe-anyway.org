#+TITLE: So how hard is solving hard lattice problems anyway? 
#+SUBTITLE:  
#+LaTeX_CLASS: beamer
...
#+LATEX_HEADER: \def\qenumworstfit{\(1/(4e)\, \beta \log(\beta) - 1/2\beta + 8\)}
...
#+DATE: 10 July 2019, AfricaCrypt@@beamer:\vfill \begin{scriptsize}Based on joint work with Alex Davidson, Amit Deo, Benjamin R. Curtis, Eamonn W. Postlethwaite, Elena Kirshanova, Fernando Virdia, Florian Göpfert, Gottfried Herold, John M. Schanck, Léo Ducas, Marc Stevens, Rachel Player, Sam Scott, Thomas Wunderer and Vlad Gheorghiu as well as the work of many other authors.\end{scriptsize}@@
...
|------------------------------+---------+-----------+-------------+----------|
|                          <r> |         |           |             |          |
|   \rogr \enumworstfit [fn:2] |     456 |       738 |         313 |      370 |
|           \enumavgfit [fn:3] |     248 |       416 |         165 |      200 |
#+BEAMER: }
#+END_CENTER
...
|                        <r> |         |           |             |          |
|       \enumworstfit [fn:2] |     456 |       738 |         313 |      370 |
|   \rogr \enumavgfit [fn:3] |     248 |       416 |         165 |      200 |
#+BEAMER: }
#+END_CENTER
...
** \(1/8 \approx 0.125\) v \(1/(2e) \approx 0.184\)
...
- In the sandpile model,footfullcite:C:HanPujSte11 as the algorithm proceeds through the indices \(i\), a “bump” accumulates from index \(i + 1\) onward.
...
#+END_EXPORT
#+BEGIN_CENTER
Preprocessing in dimension \((1+c)\cdot\beta\) for enumeration in dimension \(\beta\).[fn:4]
#+END_CENTER
...
- Implements several sieve algorithms[fn:5] (but not BDGL)
...
- Dual :: Find short \(\vec{v}\) s.t. \(\vec{v}\cdot \vec{A}\) is short.
...
[fn:4] Joint work with Shi Bai, Léo Ducas and Damien Stehlé

[fn:5] Gauss, NV, BGJ1 (BGJ with one level of filtration)
