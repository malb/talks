...
#+macro: hlr @@beamer:\rowcolor{gamechangecolor}@@
...
#+attr_latex: :height .9\textheight  :options keepaspectratio,frame
[[./privacy-pass.png]]
...
- Drown everything with \(\alert{e'}\): \(\Vert\alert{e}'\Vert \gg  \Vert\alert{e}\cdot\alert{k}  - \alert{e}'' \cdot \alert{r}\Vert\)
- Since \(q > \Vert\alert{e}'\Vert\) we have that \(q/\Vert \alert{e} \Vert\) -- the "signal to noise ratio" -- is big.
- A big signal to noise ratio makes solving LWE easier.
- This requires us to use larger secret dimensions \(n\) to compensate.
- Bandwidth cost is essentially \(n \log q\).
...
- Make \(q \gg \poly\) so that red area is negligibly small.
- Big "signal-to-noise" ratio: increasing \(n\) again.
...
- {{{fnsize(fullcite:PKC:ADDS21)}}}
- {{{fnsize(fullcite:AC:AlbGur24)}}}
- {{{fnsize(fullcite:EPRINT:ESTX24)}}}
  
** An Alternative from FHE

We do have efficient FHE: ciphertexts are typically *smaller* than messages above.
- _Simple idea:_
...
- _Problem:_ PRFs need deep circuits, deep circuits are expensive
- _Proposal:_ Use Dark Matter (weak-)PRF candidate cite:TCC:BIPSW18 \( \sum \left(\mat{A}\cdot \vec{x} \bmod 2\right) \bmod 3\) where \(\mat{A}\) is the secret key


{{{fnsize(fullcite:EC:ADDG24)}}}

** Overview of Round-Optimal OPRFs w/o Trusted Setup
...
| {{{hlr}}} AG24   | (R)LWE+SIS                       | M-M     |          114kB |   198kB |
| {{{hlr}}} ADDG24 | mod(2,3)+lattices                | M-H     |          2.5MB |    10KB |
| {{{hlr}}} ADDG24 | mod(2,3)+lattices                | M-M     |          2.5MB |   160KB |
