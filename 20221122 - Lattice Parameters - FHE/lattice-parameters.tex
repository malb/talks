...
\institute{\textbf{2022}: Royal Holloway, University of London, \textbf{2023}: King's College London \& SandboxAQ}
\usetheme{default}
...
\begin{frame}[label={sec:org9b8f020}]{The Plan}
\begin{itemize}
...
\begin{frame}[label={sec:org94dce71},fragile]{Example}
...
\label{sec:orga7cf249}
\begin{frame}[label={sec:orga0ecb29}]{Learning with Errors}
...
\begin{frame}[label={sec:org836077f}]{Learning with Errors}
Given \((\mathbf{A},\vec{c})\), find \(\vec{s}\) when
...
\begin{frame}[label={sec:orgf80ecf8},fragile]{"Having Small Entries"}
 \begin{columns}
...
\begin{frame}[label={sec:org0596444},fragile]{Estimator}
 \lstset{language=Python,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
See \url{https://github.com/WvanWoerden/NTRUFatigue} (this is "on the road map")
\end{alertblock}
\end{frame}

\section{Approaches}
\label{sec:org0cccd23}
\begin{frame}[label={sec:org096c228}]{Unique SVP/BDD: Translation}
...
\begin{frame}[label={sec:org1deee7e}]{Unique SVP: Computational Problem}
...
\begin{frame}[label={sec:org2b0d561}]{Approx SVP/SIS: Translation}
\begin{itemize}
...
\begin{frame}[label={sec:org70dbefd}]{Approx SVP: Computational Problem}
...
\begin{frame}[label={sec:org24ea9a9}]{Lattice Reduction <3 Combinatorics: Hybrid Attacks}
...
\begin{frame}[label={sec:org163f1cb}]{Binary Secrets}
\begin{columns}[t]
\begin{column}{0.5\columnwidth}
\begin{quote}
...
\begin{frame}[label={sec:org0daf858}]{Binary Secrets: Reductions}
\begin{theorem}[informal, \cite{EC:BraDot20}]
...
\label{thm:entropic-lwe}
\end{theorem}

\begin{block}{Personal Claim (Hot Take Alarm)}
...
\begin{frame}[label={sec:org8d1bb40}]{Binary Secrets: Attacks}
\begin{columns}
\begin{column}{0.5\columnwidth}
\begin{quote}
...
\begin{itemize}
\item \fullcite{EPRINT:CHHS19}
\item \fullcite{EPRINT:BLLWZ21}
\item \fullcite{EPRINT:BLLW22}
\end{itemize}
...
\begin{frame}[label={sec:org47d0f3a},fragile]{Estimator (Primal)}
 \textbf{plain uSVP}
...
\begin{frame}[label={sec:org375056a},fragile]{Estimator (Dual)}
 \textbf{plain SIS}
...
\label{sec:orgee3a9f3}
\begin{frame}[label={sec:orgf6ad4da}]{Lattice Volume}
...
\begin{frame}[label={sec:org770ab02}]{Gaussian Heuristic}
\begin{itemize}
...
\begin{frame}[label={sec:org4f46fff}]{Length of Gram--Schmidt Vectors}
...
\begin{frame}[label={sec:org10dde80},fragile]{Example}
 \lstset{language=Python,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[label={sec:orgecbb1f5},fragile]{Example - LLL}
 \lstset{language=Python,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
A = LLL.reduction(A)
M = GSO.Mat(A, update=True)
...
\begin{frame}[label={sec:org8018153}]{GSA}
\begin{center}
...
\begin{frame}[label={sec:orgf1e22d0}]{Strong Lattice Reduction: BKZ Algorithm (Block 0)}
\centering
...
\begin{frame}[label={sec:orgdb6bcf0}]{Strong Lattice Reduction: BKZ Algorithm (Block 1)}
\centering
\(\left(\begin{array}{ccccccccc}
\phantom{\pi_0(\vec{b}_0)} &
\phantom{\pi_0(\vec{b}_1)} &
...
\begin{frame}[label={sec:org68905b9}]{BKZ Algorithm}
\begin{algorithm}[H]
...
\begin{frame}[label={sec:org10b8ed6},fragile]{Quality}
 \centering
...
\begin{frame}[label={sec:orgec33d18},fragile]{Try it at Home}
 \lstset{language=Python,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
from fpylll import *
...
\begin{frame}[label={sec:org77fbeba}]{Success Condition for uSVP (Expectation)}
%\vspace{-2.6em}
...
\begin{frame}[label={sec:org5f6947d}]{Success Condition for uSVP (Observed)}
\tikzset{external/export=true}
...
\begin{frame}[label={sec:org95b0d7a}]{The GSA is a Lie: Tail Shape}
\tikzset{external/export=true}
...
\begin{frame}[label={sec:orgb69f6b8},fragile]{The GSA is a Lie: Tail Shape}
 \lstset{language=Python,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
from estimator import *
...
\begin{frame}[label={sec:orgddd0528},fragile]{Cost}
 \begin{itemize}
...
\begin{frame}[label={sec:orgaf75196},fragile]{Reading Estimator Output}
 \lstset{language=Python,label= ,caption= ,captionpos=b,numbers=none}
\begin{lstlisting}
...
\begin{frame}[label={sec:org3eaf04e}]{Q: "How do I …?"}
\begin{description}
...
\section{Solving SVP}
\label{sec:org88bf2ec}
\begin{frame}[label={sec:orgc2a31e6}]{Solving SVP}
\begin{columns}[t]
\begin{column}{0.5\columnwidth}
...
\begin{frame}[label={sec:org3ca885b}]{Enumeration I -- Pick a radius}
\begin{center}
...
\begin{frame}[label={sec:org50df711}]{Enumeration II -- Project basis}
\begin{center}
...
\begin{frame}[label={sec:org7e4faa0}]{Enumeration III -- Project lattice}
\begin{center}
...
\begin{frame}[label={sec:org8040544}]{Enumeration IV -- Enumerate projections}
\begin{center}
...
\begin{frame}[label={sec:org2531c7e}]{Enumeration V -- For each lift and enumerate}
\begin{center}
...
\begin{frame}[label={sec:org1c4b64a}]{Enumeration V -- For each lift and enumerate}
\begin{center}
...
\begin{frame}[label={sec:orgc1f0f4f}]{Enumeration VI -- Keep shortest}
\begin{center}
...
\begin{frame}[label={sec:org5aad2f9}]{Fast Enumeration}
\begin{itemize}
...
\begin{frame}[label={sec:org95d8b5e},fragile]{Practical Performance (Simulation)}
 \tikzset{external/export=true}
...
\begin{frame}[label={sec:org4638216}]{Sieving: Key Idea I}
\tikzset{external/export=true}
...
\begin{frame}[label={sec:orga6568df}]{Sieving: Key Idea II}
\tikzset{external/export=true}
...
\begin{frame}[label={sec:orgc52d828}]{Sieving: Key Idea III}
\tikzset{external/export=true}
...
\begin{frame}[label={sec:org4f032e2}]{Sieving: Basic (Gauss) Sieve Complexity}
\begin{itemize}
...
\begin{frame}[label={sec:org1d4ffe6}]{Sieving: Buckets I}
\tikzset{external/export=true}
...
\begin{frame}[label={sec:org9804f08}]{Sieving: Buckets II}
...
\begin{frame}[label={sec:org1c27545}]{Sieving: G6K}
...
\begin{frame}[label={sec:orgd54218e}]{Sieving: SVP}
\begin{center}
\begin{tikzpicture}
...
\begin{frame}[label={sec:org2c80a29}]{Darmstadt HSVP\textsubscript{1.05} Challenges}
\begin{center}
\begin{tikzpicture}
...
\begin{frame}[label={sec:org5a34203}]{GPU Sieving}
\begin{itemize}
...
\begin{frame}[label={sec:org55af8ae},fragile]{Try it at Home}
...
\begin{frame}[label={sec:orgf39a5d8},fragile]{Costing Sieves}
 \begin{columns}[t]
\begin{column}{0.5\columnwidth}
...
\label{sec:orgcb3f3bb}
\begin{frame}[label={sec:org09edcf1}]{Quantum Estimates}
\begin{description}
...
\item[{{\color{DarkBlue} \textbf{Enumeration} }}] Apply Montanaro’s quantum backtracking algorithm for quadratic speed-up.\footfullcite{AC:AonNguShe18}
\end{description}
\end{frame}

\begin{frame}[label={sec:org6dd967e}]{Quantum Sieving}
\begin{itemize}
...
\begin{frame}[label={sec:orgfc3e32a}]{Implementing Quantum Algorithms for SVP: Sieving (Underestimates)}
\tikzset{external/export=true}
...
\begin{frame}[label={sec:org4ac4632}]{Quantum estimates do not seem to matter}
\begin{itemize}
...
\begin{frame}[label={sec:org90d8b63}]{Other Approaches}
\begin{description}
\item[{BKW}] combinatorial technique, relatively efficient for small secrets
\begin{itemize}
\item Don’t need to worry about this unless secret is unusually small (e.g. ternary) and/or sparse.
\end{itemize}
\item[{Arora-Ge}] use Gröbner bases, asymptotically efficient , but large constants in the exponent
\begin{itemize}
\item Don’t need to worry about this unless you are given a lot of samples and have constant size errors
\end{itemize}
\end{description}
\end{frame}

\begin{frame}[label={sec:orgf659e37},standout]{Fin \& Obligatory "We're hiring" Slide}
\begin{center}
\Huge \alert{Thank You}
\end{center}

\begin{description}
\item[{KCL}] recruiting academic staff, (maybe postdocs) and PhD students (all areas of cryptography)
\item[{SandboxAQ}] recruiting (maybe staff), postdocs, PhD students and interns (post-quantum focused)
\end{description}
\end{frame}
